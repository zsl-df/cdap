 ...
 */
public class WrappedSparkDataframeSource<IN> extends SparkDataframeSource<IN> {
  private final SparkDataframeSource<IN> sink;
  private final Caller caller;

  public WrappedSparkDataframeSource(SparkDataframeSource<IN> sink, Caller caller) {
    this.sink = sink;
    this.caller = caller;
  }
  ...
  @Override
  public void run(final SparkExecutionPluginContext context) throws Exception {
   caller.call(new Callable<Void>() {
     @Override
     public Void call() throws Exception {
       sink.run(context);
