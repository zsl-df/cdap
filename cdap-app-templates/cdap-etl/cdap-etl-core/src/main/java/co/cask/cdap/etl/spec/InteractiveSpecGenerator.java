 ...
 */
public abstract class InteractiveSpecGenerator<C extends ETLConfig,
                                            P extends PipelineSpec,
                                            T extends PluginConfigurer> {
  ...
  protected InteractiveSpecGenerator(T configurer,
  ...
  protected void configureStages(ETLConfig config, PipelineSpec.Builder specBuilder) {
    ...
    Map<String, InteractivePipelineConfigurer<T>> pluginConfigurers = new HashMap<>(traversalOrder.size());
    ...
    for (ETLStage stage : traversalOrder) {
      ...
      pluginConfigurers.put(stageName, new InteractivePipelineConfigurer<>(configurer, stageName, engine));
    }
    ...
    for (ETLStage stage : traversalOrder) {
      String stageName = stage.getName();
      InteractivePipelineConfigurer<T> pluginConfigurer = pluginConfigurers.get(stageName);
  ...
  private ConfiguredStage configureStage(ETLStage stage, ValidatedPipeline validatedPipeline,
		  InteractivePipelineConfigurer<T> pluginConfigurer) {
    String stageName = stage.getName();
  ...
  protected PluginSpec configurePlugin(String pluginId, ETLPlugin etlPlugin,
		  InteractivePipelineConfigurer<T> pipelineConfigurer) {
