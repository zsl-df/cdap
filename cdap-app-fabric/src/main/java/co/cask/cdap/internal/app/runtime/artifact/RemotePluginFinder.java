...
/**
 * Implementation of {@link PluginFinder} that use the artifact HTTP endpoints
 * for finding plugins.
 */
public class RemotePluginFinder implements PluginFinder {
    ...
    RemotePluginFinder(CConfiguration cConf, DiscoveryServiceClient discoveryServiceClient,
            AuthenticationContext authenticationContext, LocationFactory locationFactory) {
        this.remoteClient = new RemoteClient(discoveryServiceClient, Constants.Service.APP_FABRIC_HTTP,
                new DefaultHttpRequestConfig(false), String.format("%s", Constants.Gateway.API_VERSION_3));
    ...
    public Map.Entry<ArtifactDescriptor, PluginClass> findPlugin(NamespaceId pluginNamespaceId,
            ArtifactId parentArtifactId, String pluginType, String pluginName, PluginSelector selector)
            ...
            return Retries.callWithRetries(() -> {
                ...
                for (PluginInfo info : infos) {
                    ...
                    PluginClass pluginClass = new PluginClass(info.getType(), info.getName(), info.getDescription(),
                            ...
                            info.getClassName(), info.getConfigFieldName(), info.getProperties(), info.getEndpoints());
                ...
                Location artifactLocation = getArtifactLocation(
                        Artifacts.toArtifactId(pluginNamespaceId, selected.getKey()));
                return Maps.immutableEntry(new ArtifactDescriptor(selected.getKey(), artifactLocation),
                        selected.getValue());
            }, retryStrategy);
     ...
     *
     * @param namespaceId
     *            namespace of the call happening in
     * @param parentArtifactId
     *            the parent artifact id
     * @param pluginType
     *            the plugin type to look for
     * @param pluginName
     *            the plugin name to look for
     * @return a list of {@link PluginInfo}
     * @throws IOException
     *             if it failed to get the information
     * @throws PluginNotExistsException
     *             if the given plugin type and name doesn't exist
     */
    private List<PluginInfo> getPlugins(NamespaceId namespaceId, ArtifactId parentArtifactId, String pluginType,
            String pluginName) throws IOException, PluginNotExistsException {
        HttpRequest.Builder requestBuilder = remoteClient.requestBuilder(HttpMethod.GET, String.format(
                "namespaces/%s/artifacts/%s/versions/%s/extensions/%s/plugins/%s?scope=%s&pluginScope=%s",
                namespaceId.getNamespace(), parentArtifactId.getArtifact(), parentArtifactId.getVersion(), pluginType,
                pluginName,
                NamespaceId.SYSTEM.equals(parentArtifactId.getNamespaceId()) ? ArtifactScope.SYSTEM
                        : ArtifactScope.USER,
                NamespaceId.SYSTEM.equals(namespaceId.getNamespaceId()) ? ArtifactScope.SYSTEM : ArtifactScope.USER));
        // add header if auth is enabled
        if (authorizationEnabled) {
            requestBuilder.addHeader(Constants.Security.Headers.USER_ID,
        ...
        if (response.getResponseCode() != 200) {
            throw new IllegalArgumentException("Failure in getting plugin information with type " + pluginType
                    + " and name " + pluginName + " that extends " + parentArtifactId + ". Reason is "
    ...
    private Location getArtifactLocation(ArtifactId artifactId) throws IOException {
        HttpRequest.Builder requestBuilder = remoteClient.requestBuilder(HttpMethod.GET,
        ...
        if (!location.exists()) {
            throw new IOException(String.format("Artifact Location does not exist %s for artifact %s version %s", path,
